			  ...
			  struct loop_config __user *arg)
{
	...
	pr_err("LOOP_CONFIGURE: lo=%d start\n", lo->lo_number);

	if (copy_from_user(&config, arg, sizeof(config))) {
		pr_err("LOOP_CONFIGURE: lo=%d copy_from_user failed\n", lo->lo_number);
		return -EFAULT;
	}
	...
	if (config.__reserved[0] || config.__reserved[1] ||
		...
		pr_err("LOOP_CONFIGURE: lo=%d reserved fields non-zero\n", lo->lo_number);
		return -EINVAL;
	}

	if (config.block_size && (config.block_size < 512 ||
	    config.block_size > PAGE_SIZE || !is_power_of_2(config.block_size))) {
		pr_err("LOOP_CONFIGURE: lo=%d invalid block_size=%u\n", lo->lo_number, config.block_size);
		return -EINVAL;
	}

	pr_err("LOOP_CONFIGURE: lo=%d fd=%u block_size=%u info_nonzero=%d\n",
	...
	if (err) {
		pr_err("LOOP_CONFIGURE: lo=%d loop_set_fd failed=%d\n", lo->lo_number, err);
		return err;
	}
	pr_err("LOOP_CONFIGURE: lo=%d loop_set_fd success\n", lo->lo_number);

	/* STEP 2: LOOP_SET_STATUS64 equivalent (only if info is non-zero) */
	if (memchr_inv(info, 0, sizeof(*info))) {
		err = loop_set_status(lo, info);
		if (err) {
			pr_err("LOOP_CONFIGURE: lo=%d loop_set_status failed=%d\n", lo->lo_number, err);
			goto out_clr_fd;
		}
		pr_err("LOOP_CONFIGURE: lo=%d loop_set_status success\n", lo->lo_number);
	}
	...
	if (config.block_size) {
		...
		if (err) {
			pr_err("LOOP_CONFIGURE: lo=%d loop_set_block_size failed=%d\n", lo->lo_number, err);
			goto out_clr_fd;
		}
		pr_err("LOOP_CONFIGURE: lo=%d loop_set_block_size success\n", lo->lo_number);
	}

	pr_err("LOOP_CONFIGURE: lo=%d done\n", lo->lo_number);
	return 0;
