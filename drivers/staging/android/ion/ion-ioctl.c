 ...
 * may be copied, distributed, and modified under that license.
 *
	...
	case ION_IOC_ALLOC:
	{
		...
		if (IS_ERR(handle)) {
			IONMSG(
				...
				ret);
			pr_err("[ION_DEBUG] ION_IOC_ALLOC from pid %d (%s): len=%u, align=%u, heap_mask=0x%x, flags=0x%x, ret=%ld\n",
			       current->pid, current->comm,
			       data.allocation.len, data.allocation.align,
