	...
	int write)
{
	...
	if (ctx->qid == BTAG_STORAGE_EMBEDDED)
		mtk_btag_mictx_eval_req(NULL, write, 1, len, false);
...
void mt_biolog_cmdq_isdone_end(unsigned int task_id)
{
	...
	mtk_btag_mictx_eval_tp(NULL, write, busy_time, bytes);
...
void mt_biolog_mmcqd_req_start(struct mmc_host *host)
{
	...
	if (ctx->qid == BTAG_STORAGE_EMBEDDED)
		mtk_btag_mictx_update_ctx(NULL, 1);
#endif
}
...
void mt_biolog_mmcqd_req_end(struct mmc_data *data)
{
	...
	if (ctx->qid == BTAG_STORAGE_EMBEDDED) {
		...
		mtk_btag_mictx_update_ctx(NULL, 0);
	}
...
int mt_mmc_biolog_init(void)
{
	...
	static struct mtk_btag_vops mmc_btag_vops = {
